...
## LOCATION:     /etc/pacman.d/scripts/refind-install.sh
## AUTHOR:       AISK11
## DESCRIPTION:  Refind configuration.
## DEPENDENCIES: system/coreutils system/grep system/filesystem
##               system/util-linux system/efibootmgr system/sed
...
## Copy refind application to target + fallback (<ESP>/EFI/BOOT/BOOTX64.EFI).
cp /usr/share/refind/refind_x64.efi "${TARGET}/"
cp "${TARGET}/refind_x64.efi" "${TARGET}/BOOTX64.EFI"

## Copy default refind icons and fonts to ESP and create themes directory.
cp -r /usr/share/refind/icons/ /usr/share/refind/fonts/ "${TARGET}/"
[ ! -d "${TARGET}/themes/" ] && mkdir -p "${TARGET}/themes/"

## Set up configuration file.
[ ! -f "${TARGET}/refind.conf" ] && cat << EOF > "${TARGET}/refind.conf"
## LOCATION:     ${TARGET}/refind.conf
## AUTHOR:       Generated by "/etc/pacman.d/scripts/refind-install.sh"
## DESCRIPTION:  Refind bootloader configuration.
...
## Default value: osx
use_graphics_for linux,osx,windows,grub,elilo
...
showtools hidden_tags, shell, memtest, gdisk, netboot, mok_tool, windows_recovery, apple_recovery, csr_rotate, shutdown, reboot, firmware, fwupdate, about, exit
...
extra_kernel_version_strings linux-zen,linux-hardened,linux-lts,linux
...
## Include secondary file with stanza entries.
include ./stanzas-artix.conf

## Include rEFInd theme.
...
NVRAM_ENTRY=$(efibootmgr | tr '\\\\' '/' | grep -i "${REFIND}" | \
    ...
    grep -o '[0-9]*')
BOOT_ORDER=$(efibootmgr | grep 'BootOrder:' | cut -d ' ' -f 2 | tr ',' '\n')
BOOT_ORDER_1=$(echo "${BOOT_ORDER}" | head -n 1)

## Remount efivars to be RW if RO.
grep '/sys/firmware/efi/efivars' /etc/mtab | grep -w 'ro' > /dev/null && \
    mount -o remount,rw /sys/firmware/efi/efivars

## Remove all rEFInd entries that are not 1st in UEFI NVRAM.
if [ ! -z "${NVRAM_ENTRY}" ]; then
    echo "${NVRAM_ENTRY}" | while read ENTRY; do
        if [ ! "${ENTRY}" = "${BOOT_ORDER_1}" ]; then
...
## Has rEFInd not have 1st entry in UEFI NVRAM?
if [ -z "${NVRAM_ENTRY}" ]; then
    efibootmgr -c -d "${DISK}" -p "${PARTITION}" -l "${REFIND}" -L 'rEFInd'
fi
